...
with open(out_filename, "wb") as out:
    while True:
        ...
        (is_last, data, deduplicaiton_bit) = common.decode_stopandwait_data_frame(raw_data)
        
        if random.random() < args.loss:
            print(f"Simulando perdida de frame de datos")
            continue

        ack_frame = common.encode_stopandwait_ack_frame(deduplicaiton_bit)
        if random.random() < args.loss:
        ...
        if deduplicaiton_bit != current_bit:
            continue
        out.write(data)
        out.flush()
