...
STOPANDWAIT_DATA_HEADER_FORMAT = '!bhb'
STOPANDWAIT_DATA_HEADER_SIZE = struct.calcsize(STOPANDWAIT_DATA_HEADER_FORMAT)
STOPANDWAIT_DATA_FRAME_SIZE = STOPANDWAIT_DATA_HEADER_SIZE + CHUNK_SIZE
STOPANDWAIT_ACK_FRAME_SIZE = 1

def encode_stopandwait_data_frame(is_last, data, deduplication_bit):
    header = struct.pack(
        STOPANDWAIT_DATA_HEADER_FORMAT,
        bool(is_last),
        len(data),
        deduplication_bit,
    )
    return header + pad_to_chunk_size(data)

def decode_stopandwait_data_frame(frame):
    (is_last, length, deduplication_bit) = struct.unpack(
        STOPANDWAIT_DATA_HEADER_FORMAT,
        frame[:STOPANDWAIT_DATA_HEADER_SIZE],
    )
    assert is_last in (0, 1)
    assert deduplication_bit in (0, 1)
    padded_data = frame[STOPANDWAIT_DATA_HEADER_SIZE:]
    data = padded_data[:length]
    return bool(is_last), data, deduplication_bit

def encode_stopandwait_ack_frame(sequence_bit):
    assert sequence_bit in (0, 1)
    return struct.pack('!b', sequence_bit)

def decode_stopandwait_ack_frame(frame):
    (sequence_bit,) = struct.unpack('!b', frame)
    assert sequence_bit in (0, 1)
    return sequence_bit
