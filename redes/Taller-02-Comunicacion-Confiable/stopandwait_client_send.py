...
while i < len(chunks):
    ...
    deduplicaiton_bit = i % 2 
    data_frame = common.encode_stopandwait_data_frame(is_last, chunks[i], deduplicaiton_bit)
    sock.sendto(data_frame, addr)
    try:
        raw_data = sock.recv(common.STOPANDWAIT_ACK_FRAME_SIZE)
        ack_frame = common.decode_stopandwait_ack_frame(raw_data)
        if ack_frame != deduplicaiton_bit:
            continue
