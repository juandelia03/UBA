...
\begin{parts}
    ...
    \begin{solution} %Uncomment and type your solution here
      ...
      Estas transacciones luego son organizadas por los mineros que solo deberían aceptar transacciones validas.
    \end{solution}
...
\If{$b \land (C \ne \varepsilon)$} \Comment{The chain is non-empty and meaningful w.r.t. $V(\cdot)$}
\State $\langle s, x, ctr \rangle \gets \text{head}(C)$
\State $s' \gets H(ctr, G(s, x))$
\Repeat
\State $\langle s, x, ctr \rangle \gets \text{head}(C)$
\If{$\text{validblock}_q^T (\langle s, x, ctr \rangle) \land (H(ctr, G(s,x)) = s')$}
\State $s' \gets s$ \Comment{Retain hash value}
\State $C \gets C^{[1]}$ \Comment{Remove the head from $C$}
\Else
